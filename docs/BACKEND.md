...
- **Email deliverability:** `EMAIL_FROM` is a free Gmail address rather than a verified custom domain, since the project has no domain to verify. Confirmed in production: SendGrid reports 100% delivery, but mail lands in Gmail's Spam folder rather than the inbox (Gmail treats `@gmail.com` mail not sent through Google's own servers as suspicious, especially Gmail-to-Gmail). No code fix without a paid custom domain — worth surfacing "check your spam folder" in the UI copy around forgot-password/verification.
