...
**Admin access (§9, §10):** `requireAdmin` is now a plain synchronous check on `req.user.role` — no DB call of its own, since `requireAuth` already fetched it. There is deliberately no API to grant admin; it's set exactly once, directly in the database, by the project owner on their own account — a self-service "promote to admin" endpoint would be a real privilege-escalation surface for zero benefit on a single-admin hobby project.

**Blocking admin accounts from writer actions:** a third middleware, `blockInactiveParticipant`, is likewise now a plain synchronous check — it only rejects (403) if `req.user.role === 'admin'`, since banning is already fully handled by `requireAuth` above. Applied to the specific endpoints where a user actually *acts* as a writing participant — joining Quick Match, creating/redeeming an invite, submitting a turn, responding to completion, sending a chat message.
