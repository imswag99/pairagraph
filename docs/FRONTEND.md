...
**`AdminReportsPage`** (gates the same way `AccountPage` gates on `currentUser` — `if (currentUser?.role !== 'admin') return <Navigate to="/" replace />`): fetches `GET /moderation/reports` on mount, renders each as a card (reason, reporter/reported-user name + email, optional details, a "Mark reviewed" button that calls `PATCH /moderation/reports/:id` and swaps just that card's data in local state rather than refetching the list). A report's row branches on `report.source === 'gallery'` — a gallery report (§9, §15) shows "reported a published piece" instead of the reporter→reportedUser arrow, since `reportedUser` is `null` for that source and would otherwise misleadingly render as "Deleted user". "View collaboration" used to link straight to `/collaborations/:id` — broken for an admin who isn't a participant there (`findAccessible`, `BACKEND.md` §3, rejects them with a 403). Replaced with an inline expand/collapse toggle: a `CollaborationPreview` sub-component fetches `GET /admin/collaborations/:id` (no participant check, `BACKEND.md` §10) on first expand and renders the entries and chat history read-only underneath the report row — deliberately not a reuse of `CollaborationPage`, since that page is full of participant-only actions (Report/Block/Leave, the turn composer, chat send) that make no sense for a reviewing admin. When the previewed collaboration is still published, `CollaborationPreview` also shows an "Unpublish" button (`adminService.unpublishCollaboration`, `PATCH /admin/collaborations/:id/unpublish`) — the backend action existed since the discovery phase but had no client hookup at all until this pass; found while wiring up gallery reporting, since a report needs *something* actionable for the admin to do about it.
...
- **`ThemePicker`** (`client/src/components/collaboration/ThemePicker.jsx`): a `<select>` dropdown fed from `client/src/constants/themes.js`'s `THEME_OPTIONS` — originally a pill-button row like `WritingTypePicker`, but its 6 options wrapped unpredictably in the narrow Quick Match / Invite cards, so it was switched to a dropdown for a consistently compact footprint regardless of container width. Rendered alongside `WritingTypePicker` in both `QuickMatchPanel` and `InvitePanel`'s Create tab, each wrapped in a `PickerField` (`client/src/components/collaboration/PickerField.jsx` — a small `<label>` + uppercase caption) inside a shared `bg-charcoal/5` grouping box, so the two choices read as one distinct "your options" block rather than two unlabeled button rows sitting loose above the action button.
...
Semantic landmarks (`<header>`, `<main>`, `<nav>`) were already in place. A first pass added:
...
**A second, systematic pass** (contrast, keyboard/focus order, screen-reader flow through forms — the three dimensions the first pass explicitly hadn't touched) found and fixed:

- **Contrast:** computed WCAG relative-luminance ratios against this app's actual Tailwind colors (`charcoal` `#232320`, `paper` `#faf8f3`, `indigo` `#5b5d94`) rather than guessing. `text-charcoal/60` — the single most common "secondary text" tier (34 sites) — measured 4.15:1 against `bg-paper`, below the 4.5:1 AA minimum for normal text; bumped to `text-charcoal/70` (5.68:1) sitewide, a mechanical find-and-replace with no logic change. `text-charcoal/50` (50 sites, 3.11:1) and `text-charcoal/40` (48 sites, 2.38:1) also fail AA but are **not** fixed in this pass — remediating those needs visual judgment (informational vs. genuinely decorative text) that can't be made responsibly without a browser to look at the rendered result; see `KNOWN_ISSUES.md` #10 for this as an explicit, tracked remainder, not an oversight.
- **`Modal.jsx`** (shared by every modal — Login/Register/ForgotPassword/Report): gained a manual focus trap (Tab/Shift+Tab wrapped between the panel's first/last focusable elements), Escape-to-close, focus restoration to whatever was focused before the modal opened, and `role="dialog"`/`aria-modal="true"`/`aria-labelledby` (via `useId()` on the title). Deliberately kept the existing div-based structure rather than migrating to the native `<dialog>` element — `<dialog>`'s default browser-chrome reset (border/padding/centering) is a real regression risk that can't be checked without a browser, so this fixes the underlying issue without touching the visual output at all.
- **Forms:** every input/select/textarea across the app used placeholder-only labeling (no `<label>`/`aria-label`/`id` anywhere — `AccountPage`'s display-name field didn't even have a placeholder). Added visually-hidden (`sr-only`) `<label>`s paired with `htmlFor`/`id` across `LoginModal`, `RegisterModal`, `ForgotPasswordModal`, `ReportModal`, `ResetPasswordPage`, `AccountPage` (all three sections), and `ChatPanel`'s message input — same visual appearance, real screen-reader labels underneath. `RichTextEditor`'s Tiptap contenteditable (which can't take a `<label htmlFor>` the normal way) got an `aria-label` from its existing `placeholder` prop instead; a matching visible focus ring on the contenteditable itself was considered and skipped, since it sits flush under the toolbar with no gap and a ring's rounded corners not matching the wrapper's could produce a visual seam that isn't verifiable without a browser — the wrapper's existing `focus-within` ring already gives a visible indicator.
- **Live regions:** every error `<p>` got `role="alert"`, every success/status `<p>` got `role="status"`, across all the files touched above — previously plain text a screen reader had no reason to notice appearing.
- **Skip-to-content link:** added to `AppShell.jsx` and `AdminShell.jsx` — `sr-only focus:not-sr-only`, invisible until keyboard-focused, jumps to a new `id="main-content"` on each shell's `<main>`.

**Still not done, tracked in `KNOWN_ISSUES.md` #10:** the `/50`/`/40` contrast remediation above, and any live verification of the above in an actual browser or screen reader — this environment has no browser access (a limitation already disclosed elsewhere in this project for UI work), so these fixes are implemented per WCAG's documented technique, not confirmed by hands-on testing.
...
**`GalleryPage`:** a paginated grid of cards (excerpt, writingType/theme tag, byline, publish date) with the same "Load more" pagination `CollaborationsPage`'s past-list already implements, plus writingType/theme filter chips styled like `WritingTypePicker`'s pill-button pattern but with its own local `FilterRow` (not the `ThemePicker`/`WritingTypePicker` components themselves) so an "All types"/"All themes" option can be supported, which those two don't need elsewhere.

**`GalleryItemPage`:** renders a published piece by reusing the existing `EntryList` component as-is — porting the PDF export's continuous-prose algorithm (`utils/exportCollaborationPdf.js`, §11) was considered and rejected, since that algorithm is DOM/jsPDF-specific and not reusable for an on-screen render, whereas `EntryList`'s existing turn-by-turn on-screen rendering already works unmodified here. No chat, composer, or completion controls — this is a read-only view, not a participant view. A "Report this piece" button (only shown when `currentUser` exists — reporting needs a login, matching the backend) opens the existing `ReportModal` with a new `mode="gallery"` prop, which swaps the title ("Report this piece") and posts to `moderationService.reportGalleryContent` instead of `reportUser` (`BACKEND.md` §9) — the same form, no duplicated UI.
...
**Not yet covered** (a first pass, not full coverage): the leaderboard's badge gallery, the gallery pages, the theme/writing-type pickers, `CollaborationPage` itself, admin pages, and everything socket-related. Expand incrementally as changes touch those areas, rather than trying to backfill everything at once.
...
| 17 | The admin `unpublishCollaboration` action (built during the discovery/gallery phase, `BACKEND.md` §10) had zero client-side hookup — no `adminService` method, no button anywhere — a fully working, real endpoint sitting completely unreachable from the UI | Simply never wired up when it was built; the discovery phase's frontend work focused on the writer-facing publish/consent controls and the public gallery pages, and the admin side of "someone published something bad" was left implicit rather than built | Found while adding gallery-report reporting (§15) — a report needs *something* actionable for the admin to do, which surfaced the gap. Added `adminService.unpublishCollaboration`, and an "Unpublish" button in `AdminReportsPage`'s `CollaborationPreview`, shown whenever the previewed collaboration is still published |
