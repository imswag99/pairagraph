...
Last reviewed: 2026-07-30, after adding the public discovery gallery.
...
9. **No frontend automated test coverage** — no Vitest/RTL, no Playwright. Backend has 111 passing tests in CI; frontend has none. The single biggest gap on the frontend side.
...
17. **No visitor-facing "report this piece" button on the public gallery.** The existing `Report` model requires the reporter to be a participant in the reported collaboration (`findTargetParticipant`), which doesn't fit "a random visitor reports a published piece they had no part in" — reshaping that model was judged real scope, not part of the discovery feature's first pass. The current safety valve is admin-only: an admin can force-unpublish any piece (`PATCH /admin/collaborations/:id/unpublish`) regardless of who published it. Worth revisiting once the gallery has real traffic.
